...
void processArgs(int argc,char **argv)
{
	...
	else if(argv[i][0]==SWITCHCHAR)
	{
		...
		default:
			...
			{
			    i++;
			    if(sscanf(argv[i],"%d.%d%ln",&setosmajor,&setosminor,&j)!=2)
			    {
		...
		else if(!strcmp(argv[i]+1,"subsysver"))
		{
		    ...
		    {
			i++;
			if(sscanf(argv[i],"%d.%d%ln",&setsubsysmajor,&setsubsysminor,&j)!=2)
			{
			    printf("Invalid version number %s\n",argv[i]);
			    exit(1);
			}
...
#ifdef LINUX
	printf("Linux port by Raphael Assenat\n");
#endif
