...
void BuildPEExports(long SectNum,PUCHAR objectTable,PCHAR name)
{
    ...
    for(i=0;i<expcount;i++)
    {
        if(expdefs[i].flags&EXP_ORD)
        {
            ...
            {
                printf("Duplicate export ordinal %li\n",expdefs[i].ordinal);
                exit(1);
            }
    ...
    for(i=1;i<rescount;i++)
    {
        ...
        for(j=i-1;j>=0;j--)
        {
            if(resource[j].typename)
            {
                ...
                if(wstricmp((char*)curres.typename,(char*)resource[j].typename)>0) break;
                if(wstricmp((char*)curres.typename,(char*)resource[j].typename)==0)
                {
                    if(resource[j].name)
                    {
...
void OutputWin32file(PCHAR outname)
{
    ...
    for(i=0;i<outcount;i++)
    {
        if(outlist[i] && ((outlist[i]->attr&SEG_ALIGN) !=SEG_ABS))
        {
            ...
            {
                printf("Segment overlap\n");
                printf("Next addr=%08lX,base=%08lX\n",started,outlist[i]->base);
                fclose(outfile);
                exit(1);
            }
            if(padsegments)
