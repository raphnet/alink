...
void loadcoff(FILE *objfile)
{
    ...
    if(stringPtr)
    {
        ...
        if(stringSize<4)
        {
            printf("Invalid COFF object file, bad string table size %li\n",stringSize);
            exit(1);
        }
        stringPtr+=4;
        stringSize-=4;
    ...
    if(symbolPtr && numSymbols)
    {
        ...
        for(i=0;i<numSymbols;i++)
        {
            ...
            {
                sym[i].name=(PCHAR)checkMalloc(9);
	...
	*/
	...
	if(seglist[segcount]->winFlags & WINF_ALIGN_NOPAD)
	{
	    ...
	    break;
        default:
	    printf("Invalid COFF object file, bad section alignment %08lX\n",seglist[segcount]->winFlags);
	    exit(1);
        }
	...
	if(seglist[segcount]->winFlags & WINF_COMDAT)
	{
		...
		if(sym[j].section==(i+1))
		{
		    ...
		    printf("Link alongside section %li",comdat->linkwith);
